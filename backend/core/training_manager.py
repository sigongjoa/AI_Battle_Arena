...
from backend.core.grpc_callback import GrpcCallback
...
class TrainingManager:
    ...
    def __init__(self, session_id: str):  # Removed websocket
        ...
        def _run_learning_blocking(
            self, grpc_callback
        ):  # Renamed ws_callback to grpc_callback
            """
            ...
            try:
                self.model.learn(
                    total_timesteps=100000, callback=grpc_callback
                )  # Pass grpc_callback
        ...
        async def run_grpc_stream(self):  # Renamed start_training to run_grpc_stream
            ...
            self.grpc_callback = GrpcTrainingCallback(
                metrics_queue=self.metrics_queue, loop=loop, session_id=self.session_id
            )  # New callback, stored as grpc_callback
            ...
            try:
                while self._training or not self.metrics_queue.empty():
                    try:
                        metrics_pb = await asyncio.wait_for(
                ...
                await asyncio.gather(
                    learn_task, return_exceptions=True
