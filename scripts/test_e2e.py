...
from src.rl_training.environment import FightingEnv
...
def run_test():
    ...
    try:
        # Initialize the environment without blocking for connection yet.
        env = FightingEnv(backend_peer_id=BACKEND_PEER_ID)

        # Navigate the browser to the frontend URL
        print(f"\n>>> Navigating browser to: {FRONTEND_URL}")
        # This part will be handled by the agent's navigate_page tool
        # For manual execution, you would open the URL yourself.

        # Now, wait for the frontend to connect.
        env.wait_for_connection()
        ...
        # Use temporary directory instead of hardcoded user path
        temp_dir = os.path.join(os.path.expanduser("~"), ".cache", "ai-battle-arena", "e2e")
        os.makedirs(temp_dir, exist_ok=True)
        result_file = os.path.join(temp_dir, "e2e_test_result.txt")
