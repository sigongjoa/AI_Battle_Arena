...
export class CharacterRenderer {
  ...
  constructor(container: HTMLElement, options: RendererOptions = {}) {
    ...
    const frustumSize = 600; // 캐릭터 높이(~350)보다 충분히 큼
    this.camera = new THREE.OrthographicCamera(
      ...
      0.1,                          // near
      2000                          // far (더 멀리 볼 수 있도록)
    );
    this.camera.position.set(0, 100, 500); // 캐릭터 중심(Y=100 근처)을 보도록
    this.camera.lookAt(0, 100, 0); // 캐릭터 중심을 바라봄
