...
class MockGameClient:
    def __init__(self, action_queue: queue.Queue, result_queue: queue.Queue):
        ...
        # Start the mock game loop in a separate non-daemon thread
        self._thread = threading.Thread(target=self._run_mock_game_loop, daemon=False)
        self._thread.start()
    ...
    def _run_mock_game_loop(self):
        ...
        while self._running:
            try:
                message = self.action_queue.get() # Blocks indefinitely until a message is available
                time.sleep(0.01) # Give a small delay for thread scheduling
                ...
                logger.error(f"Error in mock game loop: {e}")
